...
void SimpleIDT_Setup(uint32_t* idt, uint32_t* idtr)
{
    ...
    *ptr2 = ((uint32_t)idt);
    ...
    /*
    // The Code below basically fills up the IDT Table with Interrupt number to Interrupt Handler mappings
    // But Because You haven't started digging into Interrupts yet, Mark these commented for future.
